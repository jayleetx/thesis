...
library(here)
library(patchwork)
source(here('code', 'precinct_math.R'))
```
...
```{r}
library(purrr)
source <- st_read(here('data', 'source_zones')) %>%
  transmute(name = c('D','C','A','B'),
            lon = map_dbl(geometry, ~st_centroid(.x)[[1]]),
            lat = map_dbl(geometry, ~st_centroid(.x)[[2]]))
target <- st_read(here('data', 'target_zones')) %>%
...
```{r fig.cap = "Proportion of non-Hispanic Asian residents"}
asian_pre <- ggplot(census_props) + geom_sf(aes(fill = asian), lwd = 0) + theme_void() + ggtitle('Pre-interpolation')
asian_post <- ggplot(demo_data) + geom_sf(aes(fill = asian), lwd = 0) + theme_void() + ggtitle('Post-interpolation')

asian_pre + asian_post + plot_layout(ncol = 2)
```
...
```{r, fig.cap = 'Example error in intensive interpolation'}
ggplot(sf_wrong, aes(x = women,
                     y = women_wrong,
                     col = abs(women - women_wrong))) +
  geom_point() +
  labs(title = "Error in proportion of women",
       x = "Calculated Percentage",
       y = "Reported Percentage",
       col = "Absolute Error") +
  scale_colour_gradient(high = "#132B43", low = "#56B1F7") +
  theme_void()
# get a better color scheme for this error
# scale the error by the population of a region to show what's up
```
...
```{r fig.cap = 'Boundary intersection between shapefiles'}
# add a minimal bounding box to these to make the grid lines match up
...
prec_out + cen_out - joint_out + plot_layout(ncol = 1, heights = c(3,6))
```
...
precincts_pre <- ggplot(precincts_shape) + geom_sf() + theme_void() + labs(title = 'Precincts - original')

census_pre <- ggplot(census_shape) + geom_sf() + theme_void() + labs(title = 'Census - original')

precincts_post <- ggplot(precincts) + geom_sf() + theme_void() + labs(title = 'Precincts - bounded')

census_post <- ggplot(census) + geom_sf() + theme_void() + labs(title = 'Census - bounded')

(precincts_pre + census_pre) - (precincts_post + census_post) + plot_layout(ncol = 1)
```
...
```{r}
knitr::kable(head(sf_wide %>% arrange(desc(contest)), 10), 
